...
class Controller {
    constructor(model, view) {
        this.handler = new PanelChangeHandler(this);
    ...
    setRunnerPosition(obj) {
        ...
        const runner = range.querySelector('.slider__runner');
...
$(document).ready(function () {
    console.log(model_1.uConfiguration);
    const slider = $().slider(model_1.uConfiguration);
    sliderApi = slider;
...
class View {
    ...
    setElementCss(element, cssRules) {
        const El = element;
    ...
    createSlider(obj) {
        ...
        if (this.fetchModelProperty('scaleOn'))
            this.createScales({ parentNode: ROOT_NODE, vertical });
        return undefined;
    }
    createHorizontalScales(parentNode) {
    ...
    setRunnersDataAttributes(elements) {
    ...
    setTooltipDataAttributes(elements) {
        const collection = elements;
    ...
    onElementClickHandler(event) {
        ...
        const runnerWidth = runners[0].offsetWidth;
        let click = event.pageX - range.offsetLeft - range.clientLeft;
        let prevDiff = 10000;
        let index;
        runners.forEach((runner, i) => {
            const pos = parseInt(runner.style.left, 10);
    ...
    onRunnerMouseDownHandler(event) {
        event.preventDefault();
    ...
    onRunnerMouseMoveHandler(event) {
    ...
    onRunnersCollision(obj) {
        const { targetElement, pair, nextPosition, vertical, } = obj;
